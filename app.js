...
// ---------------- Filtering ----------------
function matchesBase(e, opts){
  opts = opts || {};
  if(!opts.ignoreAnio && state.anio!=='todos' && e.anio!=Number(state.anio)) return false;
  if(state.modo!=='todos' && e.modo!==state.modo) return false;
  if(state.tematica!=='todos' && e.tematica!==state.tematica) return false;
  if(state.proveedor && !(e.proveedores||[]).some(p=>p.includes(state.proveedor))) return false;
  return true;
}
...
// Los widgets de "Adjudicado" tienen sentido solo si el filtro de Estado no excluye ADJUDICADO
function statusAllowsAdjudicado(){
  return state.status==='todos' || state.status==='ADJUDICADO';
}
function filteredOcsAdjudicado(){
  // OCs cuyo expediente esté adjudicado, respetando filtros activos
  if(!statusAllowsAdjudicado()) return [];
  return OCS.filter(o=>{
...
// Para la comparativa 2025 vs 2026: ignora el filtro de Año a propósito (si no, un lado siempre da 0)
function filteredExpsAdjudicadoParaComparativa(){
  if(!statusAllowsAdjudicado()) return [];
  return EXPS.filter(e=>matchesBase(e, {ignoreAnio:true}) && e.status==='ADJUDICADO');
}
...
function renderCompare(){
  ...
  if(!el) return;
  const adj = filteredExpsAdjudicadoParaComparativa();
...
let expandedExp = null; // expediente actualmente desplegado (solo uno a la vez)

function statusBadge(s){
  const cls = s==='ADJUDICADO'?'adj': s==='EN PROCESO COMPRA'?'proc': s==='ADJUDIC. PEND.'?'pend':'fin';
  return `<span class="badge ${cls}">${s}</span>`;
}
function antiguedadDias(fechaStr){
  const f = new Date(fechaStr + 'T00:00:00');
  const hoy = new Date();
  hoy.setHours(0,0,0,0);
  return Math.max(0, Math.round((hoy - f) / 86400000));
}
function antiguedadBadge(dias){
  let cls = 'age-new', label;
  if(dias > 90) cls = 'age-old';
  else if(dias > 30) cls = 'age-mid';
  if(dias < 30) label = `${dias} d`;
  else if(dias < 365) label = `${Math.round(dias/30)} m`;
  else label = `${(dias/365).toFixed(1)} a`;
  return `<span class="badge ${cls}" title="${dias} días desde la fecha del expediente">${label}</span>`;
}
function renderTable(){
  ...
  rows.sort((a,b)=>{
    ...
    else if(sortKey==='antiguedad'){ va=antiguedadDias(a.fecha); vb=antiguedadDias(b.fecha); }
  ...
  document.querySelectorAll('#dataTable thead th[data-k]').forEach(th=>{
  ...
  const body = shown.map(e=>{
    const dias = antiguedadDias(e.fecha);
    const isOpen = expandedExp === e.exp;
    const detailRow = isOpen ? `
      <tr class="detail-row"><td colspan="9">
        <div class="detail-box">
          <div class="detail-label">Descripción</div>
          <div>${e.desc || '—'}</div>
          <div class="detail-label">Proveedor(es)</div>
          <div>${(e.proveedores||[]).join(', ') || '— sin proveedor asignado aún —'}</div>
        </div>
      </td></tr>` : '';
    return `
    <tr class="data-row ${isOpen?'expanded':''}" data-exp="${e.exp}">
      <td class="th-expand"><span class="expand-toggle">▶</span></td>
      <td class="mono">${e.exp}</td>
      <td class="mono">${e.fecha}</td>
      <td>${antiguedadBadge(dias)}</td>
      <td>${statusBadge(e.status)}</td>
      <td class="importe-cell">${fmtMoney(e.importe)}</td>
      <td class="mono">${e.modo}</td>
      <td>${e.tematica}</td>
      <td class="prov-cell" title="${(e.proveedores||[]).join(', ')}">${(e.proveedores||[]).join(', ') || '—'}</td>
    </tr>${detailRow}`;
  }).join('');
  ...
  document.querySelectorAll('#tbody tr.data-row').forEach(tr=>{
    tr.addEventListener('click', ()=>{
      ...
      expandedExp = (expandedExp === exp) ? null : exp;
      renderTable();
    });
  });
}
...
document.querySelectorAll('#dataTable thead th[data-k]').forEach(th=>{
  th.addEventListener('click', ()=>{
    ...
    if(sortKey===k) sortDir*=-1; else { sortKey=k; sortDir = (k==='importe'||k==='antiguedad') ? -1 : 1; }
    renderTable();
  });
});
...
function exportCsv(){
  ...
  const header = ['Expediente','Fecha','Antigüedad (días)','Modo','Tipo','Temática','Proveedor(es)','Descripción','Estado','Importe'];
  const lines = [header.join(',')];
  rows.forEach(e=>{
    lines.push([e.exp, e.fecha, antiguedadDias(e.fecha), e.modo, e.tipo, e.tematica, (e.proveedores||[]).join(' / '), e.desc, e.status, e.importe]
      .map(toCsvValue).join(','));
  });
...
function exportXlsx(){
  ...
  const rows = currentRows().map(e=>({
    Expediente:e.exp, Fecha:e.fecha, AntiguedadDias:antiguedadDias(e.fecha), Modo:e.modo, Tipo:e.tipo, Tematica:e.tematica,
